...
StudyHub系统是一个计算机专业课一体化平台，从校园师生切身需求出发，集在线作业、在线考试、在线答疑等课程管理功能于一体的综合教学平台。
...
下面主要对StudyHub各关键功能进行设计，描述其实现方式。除了文字描述以外，我们也绘制了类图、顺序图等辅助描述。 
...
下面通过过程试图（包括UML中的状态图、顺序图、活动图），分四个模块来展示功能设计流程： 
...
### 类图
StudyHub的逻辑视图部分我们采用类图来进行表示，主要包括以下几个部分：
...
使用UML构件图来表示模块，用包来表示子系统，利用连接表示模块或子系统之间的关联 
...
## 人机交互设计 
结合目前的开发进度，人机交互的展示我们主要通过原型图展示登录、课程列表、作业列表、作业编辑器四部分内容。具体内容在后续开发过程中持续更新
